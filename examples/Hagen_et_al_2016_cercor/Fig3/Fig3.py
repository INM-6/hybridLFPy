...
def plot_sim(ax, cell, synapse, grid_electrode, point_electrode, letter='a'):
    ...
    plt.plot(
        cell.tvec,
        point_electrode.LFP[0] *
        1e6,
        color=color_vec[0],
        clip_on=False)
    plt.plot(
        cell.tvec,
        point_electrode.LFP[1] *
        1e6,
        color=color_vec[1],
        clip_on=False)
    plt.axis('tight')
...
def plot_sim_tstep(
    ...
    if np.any(LFP != np.zeros(LFP.shape)):
        ...
        if cbar:
            ...
            cbar = fig.colorbar(im, cax=cax, format=FormatStrFormatter(
                '%i'))
        ...
        if show_legend:
            proxy = [
                plt.Line2D(
                    (0, 1), (0, 1), color='gray' if analysis_params.bw else plt.get_cmap(
                        'RdBu', 3)(2), ls='-', lw=3), plt.Line2D(
                    (0, 1), (0, 1), color='gray' if analysis_params.bw else plt.get_cmap(
                        'RdBu', 3)(1), ls=(
                        0, (5, 5)), lw=3), plt.Line2D(
                            (0, 1), (0, 1), color='gray' if analysis_params.bw else plt.get_cmap(
                                'RdBu', 3)(0), ls=(
                                    0, (1, 1)), lw=3), ]
    ...
    color_vec = [
        'k' if analysis_params.bw else 'b',
        'gray' if analysis_params.bw else 'g']
    for i in range(2):
        ax.plot(point_electrode_parameters['x'][i],
                point_electrode_parameters['z'][i], 'o', ms=6,
                markeredgecolor='k',
    ...
    ax1.set_ylabel(
        r'$\phi(\mathbf{r}, t)$ (nV)',
        labelpad=0)  # rotation='horizontal')
...
if __name__ == '__main__':
    ...
    cell, synapse, grid_electrode, point_electrode = run_sim(
        morphology='../morphologies/stretched/L4E_53rpy1.hoc', cell_rotation=dict(
            z=np.pi), closest_idx=dict(
            x=-100, y=0., z=650.))
    plot_sim_tstep(
        fig,
        axes[0],
