...
def plot_multi_scale_output_a(fig):
    ...
    if not os.path.isfile(os.path.join(params.savefolder, 'data_analysis',
        ...
        meanInpCurrents = getMeanInpCurrents(
            params, params.n_rec_input_spikes, os.path.join(
                params.spike_output_path, 'population_input_spikes'))
    ...
    # Inset
    if show_insets:
        ax2 = fig.add_axes([lefts[0] + axwidth - insetwidth,
                           top - insetheight, insetwidth, insetheight])
        plt.locator_params(nbins=4)
...
def plot_multi_scale_output_b(fig, X='L5E'):
    ...
    ax4 = fig.add_axes(
        [lefts[0], bottom, axwidth, top - bottom], frameon=False)
    plt.locator_params(nbins=4)
    ax4.xaxis.set_ticks([])
    ax4.yaxis.set_ticks([])
    if show_ax_labels:
    ...
    # Inset
    if show_insets:
        ax6 = fig.add_axes([lefts[1] + axwidth - insetwidth,
                           top - insetheight, insetwidth, insetheight])
        plt.locator_params(nbins=4)
    ...
    # Inset
    if show_insets:
        ax8 = fig.add_axes([lefts[2] + axwidth - insetwidth,
                           top - insetheight, insetwidth, insetheight])
        plt.locator_params(nbins=4)
