...
def fig_network_input_structure(fig,
    ...
    for i, X in enumerate(params.Y + ['TC']):
        ...
        if mlab:
            Pxx, freqs = plt.mlab.psd(
                hist, NFFT=NFFT, Fs=srate, noverlap=noverlap, window=window)
        else:
    ...
    fname = os.path.join(params.savefolder, 'RecExtElectrode_sum.h5')
    im = plot_signal_power_colorplot(
        ax4,
        params,
        fname=fname,
        transient=transient,
...
if __name__ == '__main__':
    ...
    for savefolder, letters, ttl, xlbl, bottom, top in zip(
            savefolders, letterslist, show_titles, show_xlabels, bottoms, tops):
        ...
        fig_network_input_structure(fig,
                                    params,
                                    bottom=bottom,
                                    top=top,
                                    transient=200,
